...
contains
   ...
   function amplitudData(self) result(data)
      class(NODOABB), intent(inout) :: self
      type(queue_list) :: queueD
      type(Node_tABB), pointer :: root
      character(len=:), allocatable :: data
      character(len=20) :: tempStr
   
      root => self%root
      call queueD%enqueue(root%value)
      do while (queueD%size >0)
         call queueD%dequeue()
         write(tempStr, '(I0)') queueD%IDE
         data = trim(data) //","// tempStr
         root => findNode(self%root, queueD%IDE)
         if (associated(root%left)) then
            call queueD%enqueue(root%left%value)
         end if
         if (associated(root%right)) then
            call queueD%enqueue(root%right%value)
         end if
      end do
   end function amplitudData
   recursive function findNode(root, value) result(noded)
      type(Node_tABB), pointer, intent(in) :: root
      integer, intent(in) :: value
      type(Node_tABB), pointer :: noded

      if (.not. associated(root) .or. root%value == value) then
         noded => root
      else if (value < root%value) then
         noded => findNode(root%left, value)
      else
         noded => findNode(root%right, value)
      end if
